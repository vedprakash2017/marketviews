...
class ProcessingWorker(Process):
    ...
    def __init__(self, worker_id: int, input_queue: Queue, redis_config: dict):
        super().__init__()  # Initialize the Process
    ...
    def run(self):
        ...
        """
        print(f"[Processor-{self.worker_id}] 🟢 Starting on PID {os.getpid()}")
            ...
            try:
                ...
                if result:
                    ...
                    if processed_count % 10 == 0:
                        print(f"[Processor-{self.worker_id}] ✓ Processed {processed_count} tweets")
                else:
                ...
                if e.__class__.__name__ != 'Empty':
                    error_count += 1
                    print(f"[Processor-{self.worker_id}] ❌ Error: {e}")
                continue
...
class ProcessingManager:
    ...
    def start(self, workers_count=None):
        ...
        print(f"[ProcessingManager] 🚀 Spawning {workers_count} Processing Workers...")
        ...
        print(f"[ProcessingManager] ✓ All workers started!")
        
    def stop(self):
        """Stop all workers"""
        print(f"[ProcessingManager] 🛑 Stopping all workers...")
        for worker in self.workers:
            worker.terminate()
            worker.join(timeout=5)
        print(f"[ProcessingManager] ✓ All workers stopped!")
