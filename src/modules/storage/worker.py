...
class StorageWorker:
    ...
    async def flush_buffer(self):
        ...
        try:
            ...
            print(f"[Storage] ⏳ Flushing {len(current_batch)} items...")
            ...
            if current_ids:
                ...
                print(f"[Storage] ✓ ACKed {len(current_ids)} messages")
            
            self.total_processed += len(current_batch)
            self.total_batches += 1
            
        except Exception as e:
            print(f"[Storage] ❌ Flush failed (Data stays in Redis): {e}")
    ...
    async def run(self):
        ...
        print(f"[Storage] Config: batch_size={self.BATCH_SIZE}, flush_timeout={self.FLUSH_TIMEOUT}s")
        ...
        try:
            ...
            print("[Storage] ✓ Created consumer group 'storage_group'")
        except Exception:
            print("[Storage] ✓ Consumer group 'storage_group' already exists")
        ...
        while True:
            try:
                ...
                if is_full:
                    print(f"[Storage] 📦 Buffer full ({len(self.buffer)} items), flushing...")
                    await self.flush_buffer()
                elif is_stale and self.buffer:
                    print(f"[Storage] ⏰ Timeout reached ({self.FLUSH_TIMEOUT}s), flushing {len(self.buffer)} items...")
                    await self.flush_buffer()
                ...
                if self.buffer:
                    print(f"[Storage] Flushing remaining {len(self.buffer)} items...")
                    await self.flush_buffer()
                break
            except Exception as e:
                print(f"[Storage] ❌ Crash in loop: {e}")
                await asyncio.sleep(1)
        
        # Final stats
        print(f"\n[Storage] Stats:")
        print(f"  Total processed: {self.total_processed} tweets")
